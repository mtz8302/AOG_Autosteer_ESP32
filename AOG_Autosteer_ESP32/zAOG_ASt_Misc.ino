...
void SetRelays(void)
{
    for (byte i = 0; i < 8; i++) {
        ...
        if (Set.Relay_PIN[i] < 100) {
            if (bitRead(SectGrFromAOG[0], i)) digitalWrite(Set.Relay_PIN[i], Set.Relays_ON);
            else digitalWrite(Set.Relay_PIN[i], !Set.Relays_ON);
        }
        //section 9-16
        if (Set.Relay_PIN[i + 8] < 100) {
            if (bitRead(SectGrFromAOG[1], i)) digitalWrite(Set.Relay_PIN[i + 8], Set.Relays_ON);
            else digitalWrite(Set.Relay_PIN[i + 8], !Set.Relays_ON);
        }
    }
}
                ...
                if ((Set.DataTransVia > 5) && (Set.DataTransVia < 10)) {
                    if (WiFi.status() == WL_CONNECTED) { noConnectionCounter = 0; }
                        ...
                        if (noConnectionCounter > 15) {
                            ...
                            if (Set.timeoutRouterWiFi < 60) { Set.timeoutRouterWiFi = 90; }
