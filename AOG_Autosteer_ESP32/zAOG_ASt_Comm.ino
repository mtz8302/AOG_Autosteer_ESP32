...
void getDataFromAOG()
{
	byte incommingBytes[500], tempByte;
	...
	if (Set.DataTransVia < 5) {//USB
		packetLength = Serial.available();
		if (packetLength > 0) {
			for (int i = 0; i < packetLength; i++) { incommingBytes[i] = Serial.read(); }
		}
	}
	else {
		if (Set.DataTransVia < 10) {//WiFi UDP
			if (WiFiUDPRunning) {
				packetLength = WiFiUDPFromAOG.parsePacket();
				if (packetLength > 0) {
					WiFiUDPFromAOG.read(incommingBytes, packetLength);
				}
			}
		}
		else {
			if (Set.DataTransVia == 10) {//Ethernet UDP
				if (EthUDPRunning) {
					packetLength = EthUDPFromAOG.parsePacket();
					if (packetLength) {
						EthUDPFromAOG.read(incommingBytes, packetLength);
					}  // end of Packet
				}
			}
		}
	...
	for (int i = 0; i < packetLength; i++) {
		...
		if (incomSentenceDigit < 3) {
			...
			if (incomSentenceDigit == 3) {
				...
				case SteerDataFromAOGHeader:
					if (Set.aogVersion == 17) { isSteerDataFoundV17 = true; SentenceFromAOGLength = 12; }
					else { isSteerDataFound = true; }
					break;
				case SteerSettingsFromAOGHeader:
					if (Set.aogVersion == 17) { isSteerSettingFoundV17 = true; SentenceFromAOGLength = 8; }
					else { isSteerSettingFound = true; }
					break;
				case SteerArdConfigFromAOGHeader:
					if (Set.aogVersion == 17) { isSteerArdConfigFoundV17 = true; SentenceFromAOGLength = 8; }
					else { isSteerArdConfigFound = true; }
					break;
				default:
					//Serial.println("no matching PGN");
					if (Set.aogVersion == 17) { incomSentenceDigit = 2; }
				...
				//checksum
				CRC_dataFromAOG = CRC_dataFromAOG + incommingBytes[i];
				if (incomSentenceDigit == 4) {//lenght
					if (Set.aogVersion != 17) { SentenceFromAOGLength = incommingBytes[i] + 2; }//+1 for CRC + 1for Length
					incomSentenceDigit++;
					if (Set.debugmodeDataFromAOG) {
						Serial.print(" length of incomming sentence: ");
						Serial.println(SentenceFromAOGLength - 2);
					}
				}//==4
				else
				{//>4	
					if (incomSentenceDigit == SentenceFromAOGLength + 5) { //sentence complete Length + 4 byte header + 1 length + 1 CRC - 1 (starting at 0) = 3
					//checksum and sentence complete						
					//checksum NOT WORKING!!
						/*if (Set.debugmodeDataFromAOG) {
							Serial.println("doing Checksum");
							Serial.print("incommingBytes[i] (hex) :"); Serial.print(incommingBytes[i], HEX); Serial.print(" i: "); Serial.print(i);
							Serial.print(" incomSentenceDigit: "); Serial.println(incomSentenceDigit);
							Serial.print("Checksum expected: (hex) "); Serial.println(CRC_dataFromAOG, HEX);
						}

						//if (CRC_dataFromAOG == incommingBytes[i]) {
							//Serial.println("Checksum OK");
							}
							else
							{//checksum error
	...
	/*					if (isSteerDataFound) {
							...
							if (isSteerDataFoundV17) {
								...
								if (isSteerSettingFound) {
									...
									Set.lowPWM = (float)SentenceFromAOG[7];   // read lowPWM from AgOpenGPS
									Set.minPWM = SentenceFromAOG[8]; //read the minimum amount of PWM for instant on
									Set.steerSensorCounts = SentenceFromAOG[9]; //sent as setting displayed in AOG
							//		Set.wasOffset = (SentenceFromAOG[10]);  //read was zero offset Hi
							//		Set.wasOffset |= (SentenceFromAOG[11] << 8);  //read was zero offset Lo
									Set.AckermanFix = SentenceFromAOG[12];
								...
								if (isSteerSettingFoundV17) {
									...
									Set.Ko = (float)SentenceFromAOG[7] * 0.1;       // read Ko from AgOpenGPS
									//Set.steeringPositionZero = (WAS_ZERO)+DataFromAOG[6];//read steering zero offset  
									Set.steeringPositionZero = float((Set.WebIOSteerPosZero - 127) + SentenceFromAOG[8]);//read steering zero offset  
									...
									byte checksum = 0;
									for (int i = 4; i < 12; i++) checksum += SentenceFromAOG[i];
